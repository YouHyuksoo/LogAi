...
@description
규칙 기반 이상 탐지 모듈입니다.
ClickHouse에 저장된 규칙을 기반으로 로그를 검사하고 이상을 탐지합니다.

주요 기능:
1. 규칙 로드: ClickHouse anomaly_rules 테이블에서 규칙 조회
2. 규칙 기반 탐지: 로그 레벨, 키워드, 템플릿, 빈도 기반 판정
3. 이상 탐지 결과 저장: ClickHouse anomalies 테이블에 기록
4. 상세 로그 출력: 판단 근거, 처리 과정 등 상세 정보 로깅
...
규칙 우선순위:
1. level (ERROR/CRITICAL → 무조건 이상)
2. keyword (키워드 매칭)
3. frequency (빈도 임계값 초과)
4. safe_template (정상 템플릿 화이트리스트)
...
Note: LLM 분석 요청 기능은 제거됨. 이상 탐지 결과만 저장하고 로깅합니다.
"""
...
class RuleBasedAnomalyDetector:
    ...
    Note: LLM 분석 요청은 제거됨. 이상 탐지 결과만 ClickHouse에 저장.
    """
    ...
    def detect(self):
        ...
        2. 각 로그에 대해 규칙 기반 검사
        3. 이상 발견 시 ClickHouse에 저장 및 상세 로그 출력
        4. 마지막 처리 timestamp 업데이트 (중복 방지)

        Note: LLM 분석 요청은 제거됨. 이상 탐지 결과만 저장.
        """
            ...
            for row in results:
                ...
                if result.is_anomaly:
                    ...
                    logger.warning(
                        ...
                        f"   ├─ 서비스: {service}\n"
                        f"   ├─ 로그 레벨: {level}\n"
                        f"   ├─ 템플릿 ID: {template_id}\n"
                        f"   ├─ 판단 근거: {result.rule_type}={result.rule_value}\n"
                        f"   ├─ 심각도: {result.severity.upper()}\n"
                        f"   ├─ 점수: {result.score:.2f}\n"
                        f"   ├─ 설명: {result.description}\n"
                        f"   └─ 메시지: {message[:100]}..."
                    )

                    # ClickHouse anomalies 테이블에 저장
                    details = (
                        f"rule_type: {result.rule_type} | "
                        f"rule_value: {result.rule_value} | "
