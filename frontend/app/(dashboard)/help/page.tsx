...
export default function HelpPage() {
  ...
  const faqItems: FAQItem[] = [
    {
      ...
      answer: "LogAi는 온프레미스 환경에서 실행되는 자율형 AI 로그 분석 시스템입니다. Drain3 알고리즘으로 로그 패턴을 추출하고, PyOD로 이상을 탐지하며, RAG 기반 AI가 장애 원인을 분석합니다.",
    },
    {
      question: "GPU 없이도 사용할 수 있나요?",
      answer: "네, GPU 없이도 사용 가능합니다. LLM은 OpenAI, Gemini, Mistral 등 클라우드 API를 사용할 수 있고, 임베딩은 로컬 CPU 모드(sentence-transformers)를 선택하면 됩니다. 설정 페이지에서 변경할 수 있습니다.",
    },
    {
      question: "로그가 수집되지 않아요.",
      answer: "1) Docker 컨테이너가 모두 실행 중인지 확인하세요 (docker-compose ps). 2) Vector 로그 수집기가 실행 중인지 확인하세요. 3) Consumer 프로세스가 실행 중인지 확인하세요. 인프라 페이지에서 각 서비스 상태를 확인할 수 있습니다.",
    },
    {
      question: "AI 분석 결과가 느려요.",
      answer: "온프레미스 vLLM을 사용하는 경우 GPU 메모리와 성능에 따라 달라집니다. 더 빠른 응답을 원하시면 설정에서 OpenAI나 Gemini API로 전환해보세요. Gemini는 무료 티어도 제공됩니다.",
    },
    {
      question: "Slack 알림을 설정하고 싶어요.",
      answer: "설정 페이지에서 Slack Incoming Webhook URL을 입력하세요. Slack App에서 Incoming Webhook을 생성한 후, URL을 복사해서 붙여넣으면 됩니다. 테스트 버튼으로 정상 작동하는지 확인할 수 있습니다.",
    },
    {
      question: "이상 탐지 민감도를 조절하고 싶어요.",
      answer: "설정 페이지에서 '이상 탐지 민감도' 슬라이더를 조절하세요. 낮은 값은 더 많은 이상 징후를 감지하고, 높은 값은 심각한 이상만 감지합니다. 기본값은 70%입니다.",
    },
    {
      question: "데이터를 백업하고 싶어요.",
      answer: "ClickHouse 데이터는 Docker 볼륨에 저장됩니다. docker-compose down 시 -v 옵션을 사용하지 않으면 데이터가 유지됩니다. 별도 백업이 필요하면 ClickHouse의 BACKUP 명령어를 사용하세요.",
    },
  ];
  ...
  const helpSections: HelpSection[] = [
      ...
      content: (
        <div className="space-y-4">
          <p className="text-gray-300">
            LogAi에 오신 것을 환영합니다! 이 가이드를 통해 시스템을 빠르게 시작하세요.
          </p>
          <div className="space-y-3">
            <div className={cn(
              ...
              <h4 className="font-semibold text-primary mb-2">1단계: 대시보드 확인</h4>
              <p className={cn("text-sm", theme === "dark" ? "text-gray-400" : "text-gray-600")}>
                모니터링 메뉴에서 실시간 로그 스트림과 이상 탐지 현황을 확인하세요.
              </p>
            </div>
            <div className={cn(
              "p-4 rounded-lg border",
              ...
              <h4 className="font-semibold text-primary mb-2">2단계: 분석 확인</h4>
              <p className={cn("text-sm", theme === "dark" ? "text-gray-400" : "text-gray-600")}>
            ...
            <div className={cn(
              ...
              <h4 className="font-semibold text-primary mb-2">3단계: AI 채팅</h4>
              <p className={cn("text-sm", theme === "dark" ? "text-gray-400" : "text-gray-600")}>
                AI 에이전트 채팅에서 장애 상황에 대해 질문하고 분석 결과를 받아보세요.
              </p>
            </div>
          </div>
        </div>
      ...
      content: (
        <div className="space-y-4">
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <TechStackCard
              title="AI Engine"
              items={["vLLM (Llama 3.1-8B)", "TEI (bge-m3)", "OpenAI API", "Google Gemini"]}
              theme={theme}
            />
            <TechStackCard
              title="Infrastructure"
              items={["Docker Compose", "Redpanda (Kafka)", "ClickHouse", "Qdrant", "Vector"]}
              theme={theme}
            />
          </div>
        </div>
    ...
    },
    {
      ...
      content: (
        <div className="space-y-3">
          <ShortcutItem keys={["Ctrl", "K"]} description="검색 열기" theme={theme} />
          <ShortcutItem keys={["Ctrl", "/"]} description="도움말 열기" theme={theme} />
          <ShortcutItem keys={["Ctrl", "B"]} description="사이드바 토글" theme={theme} />
          <ShortcutItem keys={["Ctrl", "D"]} description="다크 모드 토글" theme={theme} />
          <ShortcutItem keys={["Esc"]} description="모달/팝업 닫기" theme={theme} />
        </div>
      ),
    },
  ];
            ...
            {filteredFAQ.map((item, index) => (
              ...
              >
                ...
                {expandedFAQ === index && (
                  <div className={cn(
                    "p-4 border-t",
                    theme === "dark" ? "border-gray-800 text-gray-300" : "border-gray-200 text-gray-600"
                  )}>
                    {item.answer}
                  </div>
                )}
